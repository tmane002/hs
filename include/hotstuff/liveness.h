...
//                });
    ...
    void reg_receive_proposal() {
        hsc->async_wait_receive_proposal().then([this](const Proposal &prop) {
            ...
            HOTSTUFF_LOG_INFO("reg_receive_proposal(): Parent selection implementation for PaceMaker");
...
//            });
    ...
    void reg_receive_other_cluster() {
        hsc->async_wait_other_cluster().then([this](const Proposal &prop) {
            const auto &hqc = hsc->get_hqc();
            HOTSTUFF_LOG_INFO("reg_receive_other_cluster(): Parent selection implementation for PaceMaker");
            const auto &blk = prop.blk;
//            if (check_ancestry(hqc, blk) && blk->get_height() > hqc_tail->get_height())
//                hqc_tail = blk;
            reg_receive_other_cluster();
        });
    }
...
class PMWaitQC: public virtual PaceMaker {
    ...
    promise_t beat() override {
        HOTSTUFF_LOG_INFO("PMWaitQC::beat()");
        promise_t pm;
    ...
    promise_t beat_resp(ReplicaID last_proposer) override {
        HOTSTUFF_LOG_INFO("PMWaitQC::beat_resp()");
...
class PMRoundRobinProposer: virtual public PaceMaker {
    ...
    void reg_receive_proposal() {
        HOTSTUFF_LOG_INFO("reg_receive_proposal(): round-robin style proposer");
        hsc->async_wait_receive_proposal().then([this](const Proposal &prop) {
    ...
    void proposer_schedule_next() {
        if (!pending_beats.empty() && !locked)
        {
            ...
            (pm_qc_finish = hsc->async_qc_finish(last_proposed))
                .then([this, pm]() {
                    HOTSTUFF_LOG_INFO("got QC, propose a new block");
                    pm.resolve(proposer);
                });
            locked = true;
        }
    ...
    void do_new_consensus(int x, const std::vector<uint256_t> &cmds) {
        ...
        (pm_qc_manual = hsc->async_qc_finish(blk))
            .then([this, x]() {
                HOTSTUFF_LOG_INFO("Pacemaker: got QC for block %d", x);
    ...
    void rotate() {
        ...
        HOTSTUFF_LOG_INFO("Pacemaker: rotate to %d with exp_timeout: %d"
                          , proposer, int(exp_timeout));
        pm_qc_finish.reject();
        pm_wait_propose.reject();
        pm_qc_manual.reject();
        // start timer
        ...
        HOTSTUFF_LOG_INFO("Pacemaker: After doubling, exp_timeout: %d"
        , proposer, int(exp_timeout));
    ...
    void stop_rotate() {
        timer.del();
        HOTSTUFF_LOG_INFO("Pacemaker: stop rotation at %d", proposer);
        pm_qc_finish.reject();
        pm_wait_propose.reject();
        pm_qc_manual.reject();
        rotating = false;
    ...
    void on_consensus(const block_t &blk) override {
        ...
        HOTSTUFF_LOG_INFO("Timer before every consensus");

        other_cluster_timer = TimerEvent(ec, [this](TimerEvent &){ HOTSTUFF_LOG_INFO("Timer after every consensus");
        });

        other_cluster_timer.add(1);
    ...
    promise_t beat() override {
        HOTSTUFF_LOG_INFO("PMRoundRobinProposer::beat()");
