...
struct MsgTest {
    ...
    Vote vote;
    MsgTest(const Vote &);
    MsgTest(DataStream &&s): serialized(std::move(s)) {}
    void postponed_parse(HotStuffCore *hsc);
};
...
struct MsgReqBlock {
    ...
    uint32_t join_or_not = 0;

    MsgReqBlock() = default;
//    MsgReqBlock(const std::vector<uint256_t> &blk_hashes);
    MsgReqBlock(const std::vector<uint256_t> &blk_hashes, const uint32_t &join_or_not);
...
//    FetchContext(const uint256_t &ent_hash, HotStuffBase *hs);
    FetchContext(const uint256_t &ent_hash, const uint32_t &join_or_not, HotStuffBase *hs);
...
class HotStuffBase: public HotStuffCore {
    ...
    inline void MsgTest_handler(MsgTest &&, const Net::conn_t &);
...
inline void FetchContext<ENT_TYPE_CMD>::timeout_cb(TimerEvent &) {
    HOTSTUFF_LOG_INFO("cmd fetching %.10s timeout", get_hex(ent_hash).c_str());
...
template<EntityType ent_type>
FetchContext<ent_type>::FetchContext(
        const uint256_t &ent_hash, const uint32_t &join_or_not, HotStuffBase *hs):
        promise_t([](promise_t){}),
        hs(hs), ent_hash(ent_hash), join_or_not(join_or_not) {

    fetch_msg = MsgReqBlock(std::vector<uint256_t>{ent_hash}, join_or_not);
//    join_or_not = 2;
...
void FetchContext<ent_type>::send(const PeerId &replica) {
    ...
    HOTSTUFF_LOG_INFO("Asking for blocks from %d", (replica));
...
void FetchContext<ent_type>::add_replica(const PeerId &replica, bool fetch_now) {
    ...
    {
        HOTSTUFF_LOG_INFO("add_replica happening here Inner");
        send(replica);
    }
