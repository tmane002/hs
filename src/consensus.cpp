...
void HotStuffCore::sanity_check_delivered(const block_t &blk) {
    if (!blk->delivered)
        throw std::runtime_error("block not delivered 2");
}
...
block_t HotStuffCore::get_delivered_blk(const uint256_t &blk_hash) {
    ...
    if (blk == nullptr || !blk->delivered)
        throw std::runtime_error("block not delivered 1");
    return blk;
}
...
block_t HotStuffCore::on_propose(const std::vector<uint256_t> &cmds,
    ...
    HOTSTUFF_LOG_INFO("Function on_propose() START");
    if (parents.empty())
    ...
    HOTSTUFF_LOG_INFO("Function on_propose() END");
    return bnew;
}
...
//    pn.send_msg(MsgTest(), get_config().get_peer_id(proposer));
