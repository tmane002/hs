...
//    b->cluster_number = 19;
    ...
    for (auto it = commit_queue.rbegin(); it != commit_queue.rend(); it++)
    {
        ...
        if ((temp_blk_height>20000) && (temp_blk_height<20220) && (temp_blk_height%2==0))
        {
            int joining_node = 0;
        ...
        if ((temp_blk_height>20000) && (temp_blk_height<20220) && (temp_blk_height%2==1))
        {
            int joining_node = 1;
            Proposal prop_join_tent(id, blk, nullptr, cluster_id, joining_node, blk->height, 6);
            LOG_INFO("LatencyPlot: new node sending join request message") ;
...
//        if (int(blk->height)%6000 ==0 && (int(blk->height) > 0))
...
//            join_nodes();
            ...
            LOG_INFO("LatencyPlot: Sending Join Init message with height %d", int(blk->height));
//            if (leader_check )
            {
                {
...
//            if (!(cluster_id==0 && blk->height==400))
            if (!(cluster_id==0 && blk->height==-500))
                {
...
void HotStuffCore::on_qc_finish(const block_t &blk) {
    LOG_INFO("function on_qc_finish:START");
    auto it = qc_waiting.find(blk);
    if (it != qc_waiting.end())
    {
        LOG_INFO("qc_waiting: promise resolved");
    ...
    LOG_INFO("function on_qc_finish:END");

}

promise_t HotStuffCore::async_wait_proposal() {
    LOG_INFO("function async_wait_proposal:START");

    return propose_waiting.then([](const Proposal &prop) {
        LOG_INFO("function async_wait_proposal:END");
...
promise_t HotStuffCore::async_wait_other_cluster() {
    LOG_INFO("function async_wait_other_cluster:START");

    return other_cluster_waiting.then([this](const Proposal &prop) {
        LOG_INFO("function async_wait_other_cluster:END");
...
promise_t HotStuffCore::async_wait_receive_proposal() {
    LOG_INFO("function async_wait_receive_proposal:START");
    return receive_proposal_waiting.then([](const Proposal &prop) {
        LOG_INFO("function async_wait_receive_proposal:END");
...
promise_t HotStuffCore::async_hqc_update() {
    LOG_INFO("function async_hqc_update:START");

    return hqc_update_waiting.then([this]() {
        LOG_INFO("function async_hqc_update:END");
...
void HotStuffCore::on_propose_(const Proposal &prop) {
    ...
    LOG_INFO("propose_waiting: promise resolved");
    t.resolve(prop);
}
...
void HotStuffCore::on_receive_proposal_(const Proposal &prop) {
    ...
    LOG_INFO("receive_proposal_waiting: promise resolved");
    t.resolve(prop);
}
