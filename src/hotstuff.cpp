...
MsgTest::MsgTest(const Vote &vote) { serialized << vote; }
...
MsgReqBlock::MsgReqBlock(const std::vector<uint256_t> &blk_hashes, const uint32_t &join_or_not) {
    serialized << htole((uint32_t)blk_hashes.size());

    serialized << htole( (uint32_t) join_or_not);
...
//    uint32_t msgCode;
    ...
    size = letoh(size);
    join_or_not = letoh(join_or_not);

    LOG_INFO("msgCode is %d", join_or_not);
...
bool HotStuffBase::on_deliver_blk(const block_t &blk) {
    LOG_DEBUG("function on_deliver_blk(), START");
    const uint256_t &blk_hash = blk->get_hash();
    bool valid;
    ...
    {
        LOG_DEBUG("Here, the block %.10s is delivered",
    ...
    LOG_DEBUG("function on_deliver_blk(), END");
    return res;
}
...
promise_t HotStuffBase::async_fetch_blk(const uint256_t &blk_hash,
    ...
    if (it == blk_fetch_waiting.end())
    {
        ...
        it = blk_fetch_waiting.insert(
            std::make_pair(
                blk_hash,
                BlockFetchContext(blk_hash, test_join_or_not,this))).first;
    }

    if (replica != nullptr)
    {
        it->second.add_replica(*replica, fetch_now);
        HOTSTUFF_LOG_INFO("Adding replica fetch_now here Outer");
    }
...
//    if (2>1)
    ...
    HOTSTUFF_LOG_INFO("going to async fetch block inside async_deliver_blk");
...
//        const PeerId tempRep = (conn->get_peer_id(0));
        pms.push_back(async_fetch_blk(qc->get_obj_hash(), &replica));
//        pms.push_back(async_fetch_blk(qc->get_obj_hash(), *tempRep ));

        /* the parents should be delivered */

        for (const auto &phash: blk->get_parent_hashes())
        {
            HOTSTUFF_LOG_INFO("NOT delivering parent blocks");

            pms.push_back(async_deliver_blk(phash, replica));
//            pms.push_back(async_deliver_blk(phash, *tempRep));
...
void HotStuffBase::propose_handler(MsgPropose &&msg, const Net::conn_t &conn) {
    ...
    LOG_WARN("async_deliver_blk going to start here with get_id(): %d", int(get_id()));


    const PeerId &temp_peer = get_config().get_peer_id(0);
...
//    });
    ...
    promise::all(std::vector<promise_t>{
            async_deliver_blk(blk->get_hash(), temp_peer)
    }).then([this, prop = std::move(prop)]() {
        on_receive_proposal(prop);
    });
...
void HotStuffBase::MsgTest_handler(MsgTest &&msg, const Net::conn_t &conn) {
    const auto &peer = conn->get_peer_id();
    msg.postponed_parse(this);
    //auto &vote = msg.vote;
    LOG_INFO("---------xxxx---MsgTest------xxxx----------");
    return;
}
...
void HotStuffBase::req_blk_handler(MsgReqBlock &&msg, const Net::conn_t &conn) {
    ...
    uint32_t jo = msg.join_or_not;


    LOG_INFO("------ jo is %d", int(jo));
...
//        }
    ...
    promise::all(pms).then([replica, this](const promise::values_t values) {
        ...
        for (auto &v: values)
        {
            LOG_WARN("Iterating over values: %d", tempCnt);
...
//    pn.reg_handler(salticidae::generic_bind(&HotStuffBase::MsgTest_handler, this, _1, _2));
...
//            pn.send_msg(MsgTest(vote), get_config().get_peer_id(proposer));
...
void HotStuffBase::do_decide(Finality &&fin) {
    ...
    {
        HOTSTUFF_LOG_INFO("Removing entry from decision waiting queue");
        it->second(std::move(fin));
        decision_waiting.erase(it);
    }
    else
    {
        HOTSTUFF_LOG_INFO("Nothing to remove from decision waiting queue");
